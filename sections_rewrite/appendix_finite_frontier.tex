...
For the developer, I verify the required curvature throughout the domain
of alternative research policies reachable from the initial efficiency.
...
Let \(D=\overline B-B\). The state equation with a finite upper bound gives
\[
...
zero. The homogeneous Ramsey block must then follow its stable direction;
...
$(\overline k,\overline d,0)$. Its constants depend on the parameters
...
Here $\varphi=(\sigma-1)/\sigma\in(0,1)$. Since capital outgrows effective
labor, $K/(AN)$ has no finite limit. Instead normalize consumption and the
shadow value by capital and scale the efficiency gap by capital:
\[
...
$\rho>n$, as imposed in the household problem. Log utility is concave, the
household budget is linear at given prices and distributions. More explicitly,
...
The candidate's objective is finite, because its research compute is
...
counterfactual efficiencies $[b_*,\overline B]$, with $0<b_*<\overline B$.
...
date. Lemma~\ref{lem:rewrite-developer-verification} and the verified TVC
therefore establish global developer optimality without imposing
$\eta\leq1/2$. No curvature property of
the candidate is being assumed: it follows from the primitives and the
