...
The static condition can be written
\[
...
\]
uniformly. These are properties of the static map, not assumptions about
the equilibrium path. They also imply that, for any prescribed finite
return, sufficiently small $k$ and sufficiently large $B$ make $r$ exceed
that return, uniformly over those small values of $k$.
...
Euler and $r\geq-\delta$ give $\dot c/c\geq-(\delta+\rho+\gamma)$.
...
the discounted sum of wages and distributions has a finite limit.
Since wages are nonnegative, the discounted distributions alone have
a limit that is either finite or minus infinity. The latter is impossible:
the developer can stop research and obtain nonnegative operating profits.
By continuation optimality, its remaining value
...
\textit{Step 3. Future research spending is small before taking growth limits.}
...
static map give a uniform positive lower bound $\ell_a$ on $w/A$.
...
\end{equation}
for a fixed finite $J$. The omitted terminal term is nonpositive;
no discount-rate limit is used.
...
For any $j\leq\varepsilon$, a feasible path cannot enter
...
\]
At $t_1$, feasibility requires $c_{t_1}<F(\varepsilon/2)$ by the preceding
invariant-region argument. Thus $c_s<F(\varepsilon/2)$ throughout the crossing.
Uniform static convergence on the crossing interval yields numbers
$\epsilon_{t_0}\to0$ such that
\[
 \dot k_s
 =y_s-u_s-\nu k_s-c_s-m_s
...
\textit{Step 5. Research vanishes pointwise and Ramsey dynamics determine the limit.}
