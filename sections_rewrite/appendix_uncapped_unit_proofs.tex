...
\end{equation}
First suppose $\eta\leq1/2$.
This function is concave in $B$ when $(1-\alpha)\omega_X\leq1/2$.
The RSI technology $F(B,M)=\chi B^\eta M^\eta$ is jointly concave
...
For $\eta>1/2$, use only within this proof the invertible state transformation
and its current-value costate
\begin{equation}
 S=B^{1-\eta},\qquad
 \dot S=(1-\eta)\chi M^\eta,\qquad
 p=\frac{qB^\eta}{1-\eta}.
 \label{eq:rewrite-uncapped-unit-transformed-state}
\end{equation}
This is the same feasible research problem, not a different technology.
At given $K,A,L$, equation~\eqref{eq:rewrite-uncapped-unit-profit} becomes
a positive time-dependent coefficient times
\[
 S^{\frac{(1-\alpha)\omega_X}
 {[1-(1-\alpha)\omega_X](1-\eta)}}.
\]
It is concave in $S$ if
$(1-\alpha)\omega_X\leq(1-\eta)/(2-\eta)$. The maintained restrictions imply
\[
 (1-\alpha)\omega_X<(1-\eta)^2
 <\frac{1-\eta}{2-\eta}\qquad(\eta>1/2).
\]
Consequently $\pi_t(S^{1/(1-\eta)})-M+p_t(1-\eta)\chi M^\eta$
is jointly concave in $S,M$. The transformed first-order and costate
conditions follow by the chain rule from the original equations.
Moreover, $pS=qB/(1-\eta)$, so the original developer TVC implies
$D_Tp_TS_T\to0$. Applying the supporting-hyperplane argument in these
coordinates yields
\[
 \widetilde J_T-J_T
 \leq-D_Tp_T(\widetilde S_T-S_T)
 \leq\frac{D_Tq_TB_T}{1-\eta}\longrightarrow0.
\]
Thus both ranges of $\eta$ give global developer optimality against all
admissible research paths. The argument applies to any candidate satisfying
the uncapped equations and developer TVC under the proposition's
restrictions, not just the BGP.
...
restrictions, rather than impose them. For this linearization only, define
\begin{align*}
 \nu&=\frac{1-\eta}{\eta},&
