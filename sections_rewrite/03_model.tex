...
Competitive firms produce the final good from physical capital $K$, effective labor $AL$, and effective AI production services $X$. Let $0<\alpha<1$ denote the output elasticity with respect to capital, $\delta\geq0$ the depreciation rate, and $\sigma>0$ the elasticity of substitution between effective labor and effective AI production services. The parameters $\omega_L$ and $\omega_X$ denote the CES weights on effective labor and effective AI production services, respectively. Production is
\begin{align}
 Y&=K^\alpha Z^{1-\alpha},
...
To illustrate this production technology, consider a researcher writing an
academic paper. The researcher contributes time and expertise to formulate
questions, evaluate arguments, and interpret results. AI assists with
derivations, programming, and drafting, while physical capital, such as a
computer, runs numerical calculations and produces figures. AI can replace
some of the work the researcher would otherwise do, but it can also make the
researcher's expertise more productive by helping test conjectures or communicate
arguments more clearly. The researcher may revise an argument drafted by AI or ask AI to
check an argument written by the researcher. A numerical result may similarly
combine a question formulated by the researcher, code developed with AI
assistance, and calculations executed on the computer.
...
An early model of mechanization is \citet{zeira1998}, in which producers
can use more capital to reduce the labor required to produce individual
intermediate goods. I model labor and AI services as aggregate inputs,
not as alternative ways of performing a single task.
\citet{jonestonetti2026} show how a production process
with many tasks can admit a CES-like aggregate representation.
\citet{erdiletal2025gate} distinguish automating additional tasks (the
extensive margin) from increasing the services supplied in tasks that AI
can already perform (the intensive margin). I focus on expansion of
aggregate AI services without explicitly tracking the set of automatable
tasks; the fixed CES weights should therefore not be read as fractions of
tasks automated.

The complementarity case connects to the ``weak links'' mechanism in
\citet[Section~9.2.2]{aghionjonesjones2019}. In their task model,
automation allows capital to replace labor within individual tasks, while
different tasks remain complements. Tasks still performed by humans can
therefore constrain production as automated tasks expand. With $\sigma<1$,
Equation~\eqref{eq:rewrite-ces} captures a related constraint: more AI
services cannot eliminate the need for effective labor. I apply this logic
to aggregate inputs with fixed weights, rather than modeling an expanding
set of automated tasks; the two substitution elasticities are not interchangeable.
...
I use \emph{compute} to denote the computational resources used to execute the numerical operations required by AI systems \citep{sastryetal2024}. %The variables $U$ and $M$ are rates of compute use rather than stocks of processors or data centers. 
I assume that one unit of compute requires one unit of the final good, which could otherwise be consumed or invested in physical capital. Compute is therefore a rival, costly input, not a measure of AI performance.

I distinguish compute used to deploy AI from compute used to improve it. Inference compute $U$ runs existing AI systems and produces outputs used in current final-good production. Research compute $M$ is allocated to the development of better AI. It includes training and fine-tuning, but also computational experimentation, model evaluation, and other compute-intensive components of AI research. I therefore use the term \emph{research compute} rather than the narrower term \emph{training compute}. \citet{korinekmckelvey2026} make a related distinction between compute used for inference and compute used for R\&D and training.
Compute can be measured in floating-point operations; because $U$ and $M$ are
flows, their physical counterparts are operations per unit of time.

The distinction between $U$ and $M$ concerns the economic use of compute,
rather than a strict separation between inference and training: running AI
systems that conduct research also requires inference compute
\citep{davidsonetal2026}.
...
Raw compute does not by itself determine the quantity of AI services produced. The variable $B$ measures AI efficiency: AI services obtained per unit of compute. Thus $U$ and $M$ are raw computational inputs, while $BU$ and $BM$ are the corresponding flows of AI production and AI research services. This interpretation parallels measures of algorithmic efficiency based on the decline in compute required to achieve a given level of performance \citep{hernandezbrown2020,erdilbesiroglu2022}. A rise in $B$ therefore represents an improvement in the algorithms that use compute, rather than an expansion of physical computing capacity. I assume that the same $B$ augments compute in both activities.

This distinction also underlies \emph{effective compute}, which adjusts
physical compute for algorithmic improvements
\citep{hoetal2024algorithms,jones2026future}. In $BM$, $M$ is rival,
whereas the algorithmic knowledge represented by $B$ is nonrival: using it
in one computation does not diminish its usefulness in another. I apply
this distinction to AI research more broadly than the model-training
activities measured in those sources.
\citet{erdiletal2025gate} similarly distinguish the nonrival model weights
created by training from the recurring compute costs of inference.
...
\end{equation}
Equation~\eqref{eq:rewrite-ai-service} maps inference compute into the flow $X=BU$ of AI production services. Research compute analogously produces $BM$, the flow of AI research services.
...
The single-developer assumption excludes the conflict between
general-purpose AI suppliers and application developers studied by
\citet{liuwan2026}.
...
Appendix~\ref{app:rewrite-proofs} gives sufficient conditions for a research
path to maximize the present value of the developer's net profit in
...
I can now combine the decisions of the three agents and the market clearing conditions to define an equilibrium. Capital-market clearing
