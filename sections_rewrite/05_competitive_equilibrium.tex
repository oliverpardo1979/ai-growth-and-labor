\section{Competition, research incentives, and growth}
...
AI suppliers have free access to the same efficiency $B$. Entry is free,
...
take prices and the common technology as given; they do not coordinate
research to capture its economy-wide benefits. Preferences, final-good
production, and the RSI technology remain unchanged.

Competition sets $p_X=1/B$. Since $X=BU$, revenue then exactly covers
inference compute: $p_XX=U$. A supplier that finances research pays $M$
without acquiring an exclusive technology or a claim on future revenue.
Private research therefore yields no return, and its optimal level is
zero. This conclusion depends on free access to improvements, not on
competition in service provision alone. Exclusive rights to improvements
or payments for research would define a different economy.

With efficiency fixed at $B_0$, competitive input choices maximize output
net of inference compute. Define
\begin{equation}
 F_{B_0}(K,AL)
 =\max_{U\geq0}\left\{K^\alpha Z(AL,B_0U)^{1-\alpha}-U\right\}.
 \label{eq:rewrite-competitive-net-production}
\end{equation}
The function $Z$ is the CES aggregator in equation~\eqref{eq:rewrite-ces}.
The subtraction of $U$ accounts for the resources needed to supply AI
services. Capital accumulation and the household's Euler equation become
\begin{equation}
 \dot K=F_{B_0}(K,AL)-C-\delta K,\qquad
 \frac{\dot C}{C}=n+\partial_K F_{B_0}(K,AL)-\delta-\rho.
 \label{eq:rewrite-competitive-dynamics}
\end{equation}
The envelope condition gives
$\partial_K F_{B_0}=\alpha Y/K=r+\delta$.
Together with the household's transversality condition
\eqref{eq:rewrite-household-tvc}, these equations determine consumption
and capital accumulation. Competitive suppliers' optimality replaces
the monopolist's pricing, research, and shadow-price conditions.
In particular, the interior research condition is not imposed at $M=0$.
...
For every $A_0,N_0,K_0>0$ and $0<B_0<\overline B$, there is a unique competitive equilibrium
in aggregate allocations. At every date,
\begin{equation}
 M_t=0,\qquad B_t=B_0,\qquad p_{X,t}=1/B_0,\qquad \Pi_t=0.
...
When $B_0$ exceeds the competitive threshold, capital and AI services
...
returns bring the interest rate back to $\rho+\gamma$. The equality case
...
Thus the pricing advantage must be compared with the efficiency gain
achieved through research, not with the upper bound alone.
This is a conditional comparison of limiting outcomes: the local
monopoly existence result does not establish an AI-dominated trajectory
from every initial efficiency below the competitive threshold.
