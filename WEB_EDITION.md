...
2. Copy its PDF to the existing stable path.
3. Generate the HTML reader and figure assets from that same compilation.
4. Export and validate the existing simulation data; run web-edition tests.
5. Publish all formats together to GitHub Pages.
...
Generated files under `docs/generated/` and `docs/paper/` are build outputs,
not maintained copies in Git. The source hash and conversion checks are stored
in `generated/manuscript-meta.json` on the published site.
...
The CI build performs this copy automatically.
