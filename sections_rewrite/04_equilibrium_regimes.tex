...
\citet{raymookherjee2022}. In my AI-dominated regime, wages grow
permanently faster than in the labor-bottleneck regime, even as AI
efficiency approaches its upper bound.
Equations~\eqref{eq:rewrite-substitutes-ai-limits}
and~\eqref{eq:rewrite-substitutes-wage-growth} quantify this increase:
relative to the labor-bottleneck regime, the increase in wage growth
equals the increase in output-per-worker growth divided by the
...
\subsection{Monopoly pricing and the growth threshold}
\label{subsec:rewrite-monopoly-threshold}

The threshold in equation~\eqref{eq:rewrite-critical-frontier} reflects
both production technology and monopoly pricing. To isolate the effect of
pricing, I hold AI efficiency fixed and compare the monopolist's choice
with AI services supplied at marginal cost, $p_X=1/B$.

\Needspace{12\baselineskip}
\begin{proposition}[Monopoly pricing and the AI-efficiency threshold]
\label{prop:rewrite-monopoly-threshold}
Suppose $\sigma>1$. Under marginal-cost pricing, let
$\mathcal R^{\mathrm{mc}}(B)$ denote the net return on capital implied by
production and pricing as $s_X\to1$. For every $B>0$,
\begin{equation}
...
\end{equation}
The efficiency threshold $\mathcal B^{\mathrm{mc}}(\sigma)$ defined by
...
At a given AI efficiency, the monopolist charges more than the marginal
cost of supplying AI services, so final-good firms use fewer of them.
Because AI services complement physical capital in production, this
restriction also lowers output and the marginal product of capital at
given $K$ and $AL$. The restriction persists in the zero-labor-share
limit, lowering the limiting output--capital ratio and the return on
capital. A higher AI efficiency is therefore needed for that return to
exceed $\rho+\gamma$. For $\alpha=0.33$, marginal-cost pricing lowers
the required efficiency by $33.0\%$; this is a reduction in the threshold,
not an increase of that magnitude in the growth rate.

The comparison also identifies a gain from wider use of an existing AI
technology. Holding $K$, $AL$, and $B$ fixed, marginal-cost pricing
maximizes $Y-X/B$: output net of the inference compute needed to supply
AI services. It therefore leaves more resources available for consumption,
capital accumulation, or research. Yet marginal-cost pricing also
eliminates the operating profits that reward research in the model.
The incentive to remove a markup after a technology has been developed
must therefore be considered alongside its effect on the incentive to
develop that technology.

This tension raises the commitment problem illustrated by patent policy
in \citet{kydlandprescott1977}. A government may promise monopoly
protection to encourage research, but later wish to withdraw it once the
research costs are sunk. If the developer anticipates this reversal, it
cannot value future research gains using the monopoly profits assumed
here. In the present setting, continued RSI makes the trade-off relevant
...
$\mathcal B^{\mathrm{mc}}(\sigma)$ is not an optimal intervention threshold:
it identifies a return consistent with growth at $\gamma$, not the best
time to change policy. Choosing that time requires accounting for the
...
algorithms and model weights, not merely a change in the number of firms.
...
As Kremer emphasizes, incentives for further development depend on
