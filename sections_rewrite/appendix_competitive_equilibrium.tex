...
For $k>0$, the marginal product of compute in gross output,
$\partial Y/\partial U$, decreases strictly from infinity to zero as
$U$ rises from zero to infinity. The derivative of net output in
\eqref{eq:rewrite-competitive-net-production} is
$\partial Y/\partial U-1$, so there is a unique positive maximizer,
at which $\partial Y/\partial U=1$. The first-order condition is
