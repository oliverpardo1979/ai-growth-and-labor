...
function loadReader(){return readerPromise ||= (async()=>{
  ...
  document.title=`${meta.title} — Oliver Pardo`;
...
function render(){if(!data)return;const range=data.windows[state.window];shownIndices=data.times.map((_,i)=>i).filter(i=>data.times[i]>=Math.max(0,range[0])&&data.times[i]<=range[1]);if(state.window==='short')shownIndices.unshift(-1);
  ...
  const labels={alpha:'Capital output elasticity · α',discount:'Time preference · ρ',depreciation:'Depreciation · δ',eta:'Research elasticity · η',omega_x:'AI weight · ωX',population_growth:'Population growth · n',labor_productivity_growth:'Labor-augmenting progress · γ',initial_labor_productivity:'Initial labor-augmenting technology · A₀',initial_population:'Initial population · N₀'};
  const rateKeys=['discount','depreciation','population_growth','labor_productivity_growth'];
  $('#data-details').innerHTML=`<p>The explorer covers RSI activation under an existing monopoly, with two illustrative research productivities and four elasticities. The separate competition-to-monopoly experiments are available in the reader. No interpolation across parameters, no new equilibrium computation, and no extrapolation beyond year 500.</p><table>${Object.entries(data.parameters).map(([k,v])=>`<tr><td>${esc(labels[k]||k)}</td><td>${esc(rateKeys.includes(k)?(v*100).toFixed(1)+'% per year':Number(v.toPrecision(6)))}</td></tr>`).join('')}<tr><td>AI-efficiency upper bound · B̄</td><td>${d.frontier.toFixed(3)}</td></tr></table><p>Initial stocks (A₀ = N₀ = 1):</p><table><thead><tr><th>σ</th><th>K₀</th><th>B₀</th></tr></thead><tbody>${Object.entries(d.initial_stocks_by_sigma).map(([k,v])=>`<tr><td>${Number(k.replace('sigma_','').replace('_','.')).toFixed(1)}</td><td>${v[0].toFixed(3)}</td><td>${v[1].toFixed(3)}</td></tr>`).join('')}</tbody></table><p>Current CSV SHA-256: <code style="overflow-wrap:anywhere">${esc(d.provenance.csv_sha256)}</code></p><p>The finite-cap equilibrium proposition gives local existence results under its stated conditions; these simulations do not establish global uniqueness. Household inequality is not identified by functional income shares in this representative-household model.</p>`;
}
