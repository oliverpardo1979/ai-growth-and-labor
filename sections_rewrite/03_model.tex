...
Raw compute does not by itself determine the quantity of AI services produced. The variable $B$ measures AI efficiency: AI services obtained per unit of compute. Thus $U$ and $M$ are raw computational inputs, while $BU$ and $BM$ are the corresponding flows of AI production and AI research services. This interpretation parallels measures of algorithmic efficiency based on the decline in compute required to achieve a given level of performance \citep{hernandezbrown2020,erdilbesiroglu2022}. A rise in $B$ therefore represents an improvement in the algorithms that use compute, rather than an expansion of physical computing capacity.\footnote{Measures of \emph{effective compute} adjust physical compute for algorithmic improvements \citep{hoetal2024algorithms,jones2026future}. I apply this distinction beyond model training to AI research more broadly. \citet{erdiletal2025gate} similarly distinguish nonrival model weights from recurring inference-compute costs.} I assume that the same $B$ augments compute in both activities.
