...
Write $k=K/(AL)$, $c=C/(AL)$, $u=U/(AL)$, and $y=Y/(AL)$.
At $p_X=1/B_0$, final-good firms' demand condition
\eqref{eq:rewrite-ai-price} implies $\partial y/\partial u=1$.
Their choice of AI services therefore maximizes output net of compute
at a given $k$. For the proof, denote this net output per unit of
effective labor by
\begin{equation}
 f(k)=\max_{u\geq0}\left\{
 k^\alpha\left[\omega_L+\omega_X(B_0u)^{(\sigma-1)/\sigma}\right]^{
 \sigma(1-\alpha)/(\sigma-1)}-u\right\}.
 \label{eq:rewrite-competitive-net-production}
\end{equation}
For $k>0$, the marginal product of compute, $\partial y/\partial u$,
decreases strictly from infinity to zero as $u$ rises from zero to
infinity. The derivative of net output is $\partial y/\partial u-1$,
so there is a unique positive maximizer. Its first-order condition is
$u=(1-\alpha)s_Xy$. Differentiating this condition, using
...
condition~\eqref{eq:rewrite-household-tvc} become
\begin{equation}
 \dot k=G(k)-c,\qquad
...
Together, the two branches cover every $k>0$. On the left branch,
...
initial consumption makes the sets of initial choices whose paths first
exit through the two boundaries open and disjoint. Both sets are nonempty,
as shown by the endpoint initial choices. They cannot exhaust the connected
...
determines $U=(AL)u$. This compute use, the factor prices in
\eqref{eq:rewrite-capital-return} and \eqref{eq:rewrite-wage}, and
$p_X=1/B_0$ decentralize this allocation.
...
\eqref{eq:rewrite-competitive-capital-growth} proves the growth limits
in part~(ii). Equation
\eqref{eq:rewrite-competitive-net-limits} gives the interest-rate limit,
and $wL/Y=(1-\alpha)(1-s_X)\to0$ gives the labor-share limit.

At the boundary $B_0=\mathcal B^{\mathrm{mc}}(\sigma)$, the same
calculations give $g_{Y/N},g_w\to\gamma$, $r\to\rho+\gamma$, and
$wL/Y\to0$. The labor share therefore vanishes without a permanent
growth premium.
\end{proof}
