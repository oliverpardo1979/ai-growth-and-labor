...
\end{equation}
Let $\mathcal R(B)$ denote the net return on capital in this zero-labor-share limit at a given $B$. Substituting the limiting output--capital ratio in
...
By the Euler equation \eqref{eq:rewrite-euler}, consumption per person grows at $r-\rho$. If this growth rate converges to $\gamma$, the interest rate must converge to $\rho+\gamma$. Define $\mathcal B(\sigma)$ as the AI-efficiency level at which the zero-labor-share return equals this benchmark:
\begin{equation}
 \begin{aligned}
...
I omit the case $0<\sigma<1$ with
$\overline B<\mathcal B(\sigma)$ from
Proposition~\ref{prop:rewrite-equilibrium-regimes} because it describes
an AI production bottleneck induced by a sufficiently low upper bound
on AI efficiency. Proposition~\ref{prop:rewrite-low-cap-complements}
in Appendix~\ref{app:rewrite-proofs} shows that, in any equilibrium
in this region, AI services become scarce relative to effective labor
and long-run average output-per-worker growth is below the exogenous
rate $\gamma$ of labor-augmenting technological progress.
...
AI efficiency improves in each of these cases, including complementarity.
In the complementary labor-bottleneck equilibria, expanding AI services
cannot substitute for the continued expansion of effective labor. As
AI-efficiency gains vanish, output per worker and wages therefore return
to growth at $\gamma$. A positive limiting labor share and the absence
of a permanent growth acceleration are two consequences of this
production bottleneck.
Substitution is needed for a permanent increase in growth under an upper
bound, not for research to improve AI efficiency.
Moreover, $\sigma>1$ is not sufficient to escape the labor bottleneck: if
...
Capital and inference compute can expand together, sustaining output-per-worker growth above
...
\citet{erdiletal2025gate} also discuss how expanding digital-worker services
alongside physical capital can generate $AK$-type dynamics. Here, the
corresponding growth regime arises under fixed aggregate production weights
and private research and pricing decisions.
RSI can help the economy reach an efficiency at which this mechanism
supports growth faster than $\gamma$; continuing efficiency improvements
are not necessary to sustain it. Section~\ref{sec:rewrite-competition}
shows that sufficiently high initial efficiency also allows this growth
mechanism to operate under competition without RSI.

This joint expansion also explains the higher long-run interest rate.
In the labor-bottleneck regime, the interest rate converges to
$\rho+\gamma$. In the AI-dominated regime, however, AI
services expand alongside capital, so production need not be constrained
by the growth of effective labor. The output--capital ratio converges to
a level at which $r=\alpha Y/K-\delta$ remains above $\rho+\gamma$:
accumulating more capital does not restore the labor-bottleneck return.
...
The proof of Proposition~\ref{prop:rewrite-equilibrium-regimes} first removes the long-run trends from growing quantities and shrinking efficiency gaps. For each parameter configuration covered by the proposition, I identify candidate stationary values of these normalized variables. I then construct equilibrium trajectories from every sufficiently nearby initial state and verify convergence to those values. %The initial stocks need not lie on a balanced-growth path.
...
To see why wages accelerate even as labor's share vanishes, I rewrite
the wage condition~\eqref{eq:rewrite-wage} using the CES aggregator:
\begin{equation}
 w=(1-\alpha)\omega_L A
 \left(\frac KZ\right)^\alpha
 \left(\frac Z{AL}\right)^{1/\sigma}.
 \label{eq:rewrite-wage-scale}
\end{equation}
In the AI-dominated regime, capital and AI services expand together,
so $K/Z$ converges to a positive constant while $Z/(AL)$ keeps growing.
Although labor accounts for a declining share of output, its marginal
product grows faster than $A$: workers participate in an expanding
production process and remain imperfectly replaceable by AI.
The joint expansion of capital and AI services is essential to this
argument; it is not the effect of increasing AI services while holding
capital fixed.

Equations~\eqref{eq:rewrite-substitutes-ai-limits}
and~\eqref{eq:rewrite-substitutes-wage-growth} make the distribution of
this growth acceleration explicit:
\begin{equation}
 \lim_{t\to\infty}g_w-\gamma
 =\frac{1}{\sigma}
 \left(\lim_{t\to\infty}g_{Y/N}-\gamma\right).
 \label{eq:rewrite-wage-growth-premium}
\end{equation}
For $1<\sigma<\infty$, wages receive a positive fraction of the
additional output-per-worker growth, but not all of it. This explains
both their permanent acceleration and the vanishing labor share.
For example, long-run output-per-worker growth of $4.0\%$, with
$\gamma=1.0\%$ and $\sigma=1.5$, implies wage growth of $3.0\%$.
These numbers illustrate the formula, not an additional simulation.

The wage-growth mechanism has a direct antecedent in the CES example
of \citet[pp.~619, 623--624]{palivoskaragiannis2010}. Combining their
production function with marginal-product wage determination implies
that wages grow at a fraction $1/\sigma$ of output-per-worker growth,
while labor's share vanishes. In their setting, substitution is between
capital and labor, and technology is fixed. Here the analogous relationship applies
to growth above $\gamma$, and AI efficiency and pricing determine
whether that additional growth can be sustained.

Perfect substitution clarifies why finite substitutability matters for
the wage-growth premium. Holding the other parameters fixed and taking
$\sigma\to\infty$ across the AI-dominated limits, the return
$\mathcal R(\overline B)$ remains bounded, so
equation~\eqref{eq:rewrite-wage-growth-premium} gives $g_w\to\gamma$
in this limit of long-run growth rates. In the corresponding linear
aggregator, $Z=\omega_L AL+\omega_X X$, the input-price conditions imply
$w/A=(\omega_L/\omega_X)p_X$: effective labor and AI services can
replace each other at a fixed rate. As AI efficiency and the price of
AI services approach constants, additional AI use no longer raises
the relative price of effective labor. Wage growth then reflects
labor-augmenting technological progress alone. This is a limiting
production and pricing argument, not an equilibrium-existence result
at $\sigma=\infty$. Nor does the finite-elasticity result imply that
a higher $\sigma$ always raises wage growth: it also changes output
growth and reduces the fraction of its acceleration received as wages.
