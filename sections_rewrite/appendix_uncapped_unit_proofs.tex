...
I use the reference BGP rates and allocation ratios derived in
Appendix~\ref{app:rewrite-uncapped-unit}, denoted by a superscript $*$.
Here $k^*=K/Y$ is a capital--output ratio, rather than capital per unit
of effective labor. I first verify the reference BGP and then establish
convergence from nearby initial stocks.

\textit{The reference BGP.}
First, all candidate ratios are positive. The denominator of the research share $m^*$,
...
It remains to establish global optimality, rather than only satisfaction of
the necessary conditions. At given paths of $K,A,L$, static optimization gives
\begin{equation}
 \begin{split}
...
alternative with the same $K_0$ and the same given prices and developer net profit,
\[
 \int_0^T e^{-\rho t}N_t
...
\]
This establishes global household optimality.
...
To establish nonsingularity of the projection onto the two stocks, write a
stable eigenvector in transformed coordinates as $(x,y,z,u)'$.
Its second component cannot be zero:
