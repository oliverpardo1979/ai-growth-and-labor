...
\end{equation}
Local integrability of research expenditure makes the exponent finite at every finite date, so
\(D_t>0\).
...
by $AN$, respectively. Construct their candidate limiting values sequentially:
\begin{align}
 \overline x&=
...
Complete the candidate long-run allocation by setting
\begin{equation}
 \overline M=
...
In either construction let $\overline g$ denote the limiting aggregate
...
first-order conditions and state equations. The result is local only in its set of initial
stocks. For $\sigma>1$, at $\overline B=\mathcal B(\sigma)$ the positive-labor-share limiting
